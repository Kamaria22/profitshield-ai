...
export default function WelcomeChecklist() {
  ...
  return (
    <Card className="future-panel mb-4 overflow-hidden border-white/8 bg-white/[0.03]">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base flex items-center gap-2 text-white">
            Launch Checklist
            <span className="text-sm font-normal text-emerald-300">{completedCount}/{CHECKLIST.length} done</span>
          </CardTitle>
      ...
      <CardContent className="pt-0">
        ...
        {CHECKLIST.map(item => (
          <div key={item.id} className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.02] px-3 py-2">
            ...
            <Link
              ...
              className={`text-sm flex-1 transition-colors ${completed[item.id] ? 'line-through text-slate-500' : 'text-slate-200 hover:text-emerald-300'}`}
            >
              {item.label}
            </Link>
