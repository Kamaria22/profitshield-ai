...
function AlertItem({ title, impact, action }) {
  return (
  ...
  return (
    <div className="space-y-3">
      <section className="dashboard-panel">
        ...
        <div className="mt-3 space-y-3">
          {visibleRisks.length ? visibleRisks.map((risk) => (
            <AlertItem key={`${risk.title}-${risk.action}`} {...risk} />
          )) : (
      ...
      <section className="dashboard-panel">
        ...
        <div className="mt-3 space-y-3">
          {visibleAlerts.length ? visibleAlerts.map((alert) => (
            <AlertItem key={`${alert.title}-${alert.action}`} {...alert} />
          )) : (
      ...
      <section className="dashboard-panel">
        ...
        <div className="mt-3 space-y-3">
          {tasks.map((task) => (
            <AlertItem key={`${task.title}-${task.action}`} {...task} />
          ))}
        </div>
      </section>
    </div>
